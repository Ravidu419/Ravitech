...
const Home = () => {
  ...
  const [filteredProducts, setFilteredProducts] = useState([]); // Search එක සඳහා වෙනම state එකක්
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setFilteredProducts(res.data); // මුලින්ම ඔක්කොම පෙන්වනවා
      } catch (err) {
  ...
  // 🔍 Navbar එකෙන් එන Query එක අනුව Filter කරන function එක
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      <Navbar onSearch={handleSearch} /> {/* Navbar එකට handleSearch එක දුන්නා */}
        ...
        <div className="flex justify-between items-end mt-10 mb-12">
          ...
          <div className="hidden md:flex flex-col items-end gap-2">
            <p className="text-slate-400 font-medium text-sm">
              {filteredProducts.length} items found for you
            </p>
          </div>
        </div>

        {/* Product Grid - මෙතන දැන් filteredProducts පාවිච්චි කරනවා */}
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="mt-auto flex gap-3">
                  <button 
                    ...
                    className="flex-1 h-12 bg-blue-600 text-white rounded-xl font-bold text-[11px] flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shadow-md shadow-blue-100"
                  >
                  ...
                  <button 
                    ...
                    className="w-12 h-12 bg-slate-100 text-slate-600 rounded-xl flex items-center justify-center hover:bg-slate-200 hover:text-slate-900 transition-all border border-slate-200"
                    title="View Details"
                  >
