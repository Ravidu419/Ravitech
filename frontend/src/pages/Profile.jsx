...
const Profile = () => {
  ...
  const [orders, setOrders] = useState([]); // Orders තියාගන්න
  ...
  useEffect(() => {
    ...
    if (!userData) {
      ...
      fetchOrders(parsedUser._id); // Order History ගෙන්වා ගැනීම
    }
  }, [navigate]);
      ...
      <div className="max-w-4xl mx-auto p-6 mt-10">
        ...
        <div className="bg-white rounded-3xl shadow-lg overflow-hidden mb-10">
          ...
          <div className="px-8 pb-8">
            ...
            {isEditing ? (
              <form onSubmit={handleUpdate} className="space-y-4 bg-slate-50 p-6 rounded-2xl">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Update Name</label>
                  <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full p-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-600"/>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">New Password (Optional)</label>
                  <input type="password" placeholder="Leave blank to keep current" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} className="w-full p-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-600"/>
                </div>
                <div className="flex gap-3 mt-4">
                  <button type="submit" className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold">Save Changes</button>
                  <button type="button" onClick={() => setIsEditing(false)} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-bold">Cancel</button>
                </div>
              </form>
            ) : (
        ...
        <h3 className="text-2xl font-black text-slate-800 mb-6">Order History 🕒</h3>
        <div className="space-y-4">
          {orders.length === 0 ? (
            <div className="bg-white p-10 rounded-2xl text-center text-slate-400">No orders found yet.</div>
          ) : (
            orders.map((order) => (
              <div key={order._id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                  <p className="text-sm text-slate-400 font-bold">Order ID: #{order._id.slice(-6).toUpperCase()}</p>
