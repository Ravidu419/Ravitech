...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        // යූසර්ගේ නම සහ ඊමේල් බ්‍රවුසරයේ සේව් කරනවා
        localStorage.setItem('user', JSON.stringify(res.data.user));

        navigate('/home'); 
      }
    } catch (err) {
      // Backend එකෙන් එන වැරදි දත්ත මැසේජ් එක පෙන්වනවා
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 relative">
      <div className="bg-white w-full max-w-md p-10 rounded-[2.5rem] shadow-xl z-10 border-2 border-blue-600">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-black text-slate-800 tracking-tight">
            RAVI<span className="text-blue-600">TECH</span>.com
          </h1>
        ...
        <div className="mt-10 text-center">
          <p className="text-slate-500 text-sm font-medium">
            New here? 
            <span onClick={() => navigate('/signup')} className="text-blue-600 font-bold ml-2 cursor-pointer hover:underline">
              Create an account
            </span>
          </p>
        </div>
